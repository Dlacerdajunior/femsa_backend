...
namespace FemsaEP.Controllers
{
    public class MobileBridgeController : Controller
    {
      ...
      //  string local = "http://localhost:51286/";
        ...
        public void procuraPecaMobile(String query, int? id_ativacao, int? id_cliente)
        {
            using (var client = new HttpClient())
            {
                client.BaseAddress = new Uri(local);
        ...
        public void criaAtivacaoMobile([Bind(Include = "ZCLIENTE, id_projeto")] ativacao atv)
        {
                ...
                var content = new FormUrlEncodedContent(new[] 
            {
                ...
                new KeyValuePair<string, string>("id_projeto", atv.id_projeto.ToString())
            });
        ...
        public void listaMensagensMobile(int? id_ativacao, int? id_usuario, int? id_cliente)
        {
            using (var client = new HttpClient())
            {
                client.BaseAddress = new Uri(local);
        ...
        public void uploadDadosCliente(List<cliente_SAP> cliente)
        {
            ...
            foreach (var item in cliente)
            {
                using (var client = new HttpClient())
                {
                    ...
                    var content = new FormUrlEncodedContent(new[] 
                        {
                            new KeyValuePair<string, string>("id", item.ZCLIENTE.ToString()),
                            new KeyValuePair<string, string>("status_ativacao", item.status_ativacao.ToString()),
                            new KeyValuePair<string, string>("motivo_id", item.motivo_id.ToString()),
                            new KeyValuePair<string, string>("ativacao_ultima_atualizacao", item.ativacao_ultima_atualizacao.ToString()),
                            new KeyValuePair<string, string>("motivotxt", item.motivotxt.ToString()),
                            new KeyValuePair<string, string>("observacao", item.observacao.ToString()),
                        });
                    var result = client.PostAsync("/Mobile/uploadDadosCliente2", content).Result;
                    string resultContent = result.Content.ReadAsStringAsync().Result;
                    Response.Write(resultContent);
                }
            }
        ...
        public int uploadDadosClienteProjeto2(List<cliente_projeto> cliente_projeto)
        {
            foreach (var item in cliente_projeto)
            {
                using (var client = new HttpClient())
                {
                    ...
                    var content = new FormUrlEncodedContent(new[] 
                        {
                            new KeyValuePair<string, string>("id_cliente", item.ZCLIENTE.ToString()),
        ...
        public void listaContrapartidasBackoffice(int? id_ativacao, int? id_cliente)
        {
