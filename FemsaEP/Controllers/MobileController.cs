...
namespace FemsaEP.Controllers
{
    ...
    public class MobileController : Controller
    {
        ...
        public ActionResult ativacaoMobile(string id)
        {
            var itemAtivacao = db.ativacao.Where(ativacao => ativacao.ZCLIENTE == id);
        ...
        public ActionResult listaPecasMobile(int? id_ativacao, string id_cliente)
        {
            ...
            //trás cluster do cliente
            var id_cliente2 = id_cliente.ToString();
            var cluster_cliente = db.cliente_cluster_SAP.Where(cc => cc.ZCLIENTE == id_cliente);
            ...
            foreach (var item in cluster_cliente)
            {
                ...
                foreach (var item2 in rsPeca)
                {
                    ...
                    list.Add(new
                    {
                        ...
                        @peca_id = item2.peca_SAP == null ? "" : item2.peca_SAP.ZCODPECA.ToString(),
                        ...
                        @peca_codpeca = peca.peca_SAP == null ? "" :peca.ZCODPIEZA.ToString(),
            ...
            foreach (var item in cc)
            {
                ...
                if (rspeca != null)
                {
                    foreach (var peca in rspeca)
                    {
                        ...
                        if (pecaativ != null && pecainfo != null)
                        {
                            list.Add(new
                            {
        ...
        public ActionResult valorTotalAtivacaoMobile(string ZCLIENTE)
        {
        ...
        public ActionResult fotosAtivacaoMobile(int? id_ativacao, int? etapa)
        {
            ...
            if (id_ativacao != null)
            {
                ...
                if (serv != null)
                {
                    ...
                    {
                        var valor = db.caracteristica_de_peca_SAP.Where(a => a.ZCODIPECA == item.ZCODPECA).FirstOrDefault();
                        var foto = "";
        ...
        public ActionResult criaAtivacaoMobile([Bind(Include = "ZCLIENTE, id_projeto")] ativacao atv)
        {
            ...
            if (atv.ZCLIENTE != null)
            {
                ...
                atv.criacao = date;
                atv.situacao = "P";
                var idcliente = atv.ZCLIENTE.ToString();
        ...
        public ActionResult salvarFotoAtivacaoMobile(ativacao_foto ativacao_foto)
        {
            ...
            if (countativ == countpeca)
            {
                ...
                if (result.id_projeto == 3)
                {
                    result.status = 1;
                    result.cliente_SAP.status_ativacao = 1;
                }
                db.SaveChanges();
        ...
        public ActionResult listaMensagensMobile(int? id_ativacao, int? id_usuario, string id_cliente)
        {
           if (id_cliente != null)
            {
                var id_cliente2 = id_cliente.ToString();
                var mensagens1 = db.mensagem.Where(mensagem => mensagem.ativacao.cliente_SAP.ZCLIENTE == id_cliente2).OrderBy(m => m.hora);

                var msgs = new List<dynamic>();

                foreach (var msg in mensagens1)
                {
                    msgs.Add(new
                    {
                        @texto = msg.texto,
            ...
            if (id_ativacao != null)
            {
                var mensagens2 = db.mensagem.Where(mensagem => mensagem.ativacao.id == id_ativacao);

                var msgs = new List<dynamic>();

                foreach (var msg in mensagens2)
                {
                    msgs.Add(new
                    {
                        @texto = msg.texto,
            ...
            if (id_usuario != null) {
                var mensagens = db.mensagem.Where(m => m.ZCODCOLAB == id_usuario || m.ZCODCOLAB == null);
             ...
             return Json(new { status = 0, msg = "Nenhuma mensagem foi encontrada."}, JsonRequestBehavior.AllowGet); 
        ...
        public ActionResult listaProjetoClienteMobile(string id_cliente)
        {
          //  var id_cliente2 = id_cliente.ToString();
            ...
            var listaProjetos = db.cliente_projeto.Where(p => p.ZCLIENTE == id_cliente);
        ...
        public int uploadDadosClienteProjeto2([Bind(Include = "ZCLIENTE, id_projeto")] cliente_projeto cliente_projeto)
        {
        ...
        public int uploadDadosCliente(cliente_SAP cliente)
        {
            if (cliente != null)
            {
                //foreach (var item in cliente)
                //{
                var clientex = db.cliente_SAP.FirstOrDefault(c => c.ZCLIENTE == cliente.ZCLIENTE);
                clientex.status_ativacao = cliente.status_ativacao;
                clientex.observacao = cliente.observacao;
                clientex.carta_foto = cliente.carta_foto;
                clientex.motivotxt = cliente.motivotxt;
                clientex.pendente_retorno = cliente.pendente_retorno;
                /*clientex.motivo_retorno = cliente.motivo_retorno;
                clientex.recusa_foto = cliente.recusa_foto;
                if (cliente.motivo_id != null)
                    {
                        clientex.motivo_id = Convert.ToInt32(cliente.motivo_id);
                    }
                    //clientex.ativacao_ultima_atualizacao = item.ativacao_ultima_atualizacao;
                clientex.motivotxt = cliente.motivotxt == null ? "" : cliente.motivotxt.ToString();*/
                    db.SaveChanges();
                //}
            }
        ...
        public void listaContrapartidasBackoffice(int? id_ativacao, string id_cliente)
        {
            var htmlretorno = "";
          //  var id_cliente2 = id_cliente.ToString();
            var projetos = db.cliente_projeto.Where(c => c.ZCLIENTE == id_cliente);
            ...
            foreach (var item in projetos)
            {
                ...
                for (int countetapa = 0; countetapa < 12; countetapa++)
                {
                    ...
                    {
                        var contrapartidas = db.contrapartida_ativacoes.Where(c => c.id_ativacao == id_ativacao && c.etapa == countetapa && c.ZCODCONPA == itemcount.ZCODCONPA);
                    ...
                    if (validarcontra >= 1)
                    {
                        ...
                        {
                            var contrapartidas = db.contrapartida_ativacoes.Where(c => c.id_ativacao == id_ativacao && c.etapa == countetapa && c.ZCODCONPA == item2.ZCODCONPA);
                            if (contrapartidas.Count() >= 1)
                            {
